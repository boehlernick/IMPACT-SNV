#!/usr/bin/env Rscript
# favor_flat_to_seqarray_gds.R
#
# Convert a flattened FAVOR preview/intermediate parquet file produced by
# flatten_favor_for_gds.py into a SeqArray-compatible GDS file for the IMPACT-SNV
# workflow.
#
# Implementation strategy:
#   1. Read the flat parquet.
#   2. Write a temporary one-sample VCF containing GT and DS.
#   3. Use SeqArray::seqVCF2GDS() to create the correct internal SeqArray layout.
#   4. Re-open the GDS and append flattened FAVOR/IMPACT annotations.
#
# This script avoids direct writes to internal SeqArray nodes like genotype/data.
# SeqArray owns those internal structures; seqVCF2GDS() is used to construct them
# safely from a minimal VCF.

if (requireNamespace("renv", quietly = TRUE)) {
  tryCatch({
    renv::activate()
  }, error = function(e) {
    message("renv activation failed: ", e$message)
  })
}
...
option_list <- list(
  make_option(c("-i", "--input"), dest = "input", type = "character",
              help = "Input flat parquet from flatten_favor_for_gds.py"),
  make_option(c("-o", "--output"), dest = "output", type = "character",
              help = "Output SeqArray GDS path"),
  make_option(c("-s", "--sample-id"), dest = "sample_id", type = "character", default = NULL,
              help = "Sample ID. If omitted, inferred from input sample_id column"),
  make_option(c("--force"), dest = "force", action = "store_true", default = FALSE,
              help = "Overwrite output GDS if it exists"),
  make_option(c("--keep-temp-vcf"), dest = "keep_temp_vcf", action = "store_true", default = FALSE,
              help = "Keep the temporary VCF used to create the SeqArray GDS"),
  make_option(c("--temp-vcf"), dest = "temp_vcf", type = "character", default = NULL,
              help = "Optional explicit temporary VCF path; useful for debugging"),
  make_option(c("--no-optimize"), dest = "no_optimize", action = "store_true", default = FALSE,
              help = "Skip seqOptimize() after writing annotations"),
  make_option(c("--storage-option"), dest = "storage_option", type = "character", default = "ZIP_RA",
              help = "SeqArray storage option name, e.g. ZIP_RA or LZMA_RA"),
...
vcf_scalar <- function(x, default = ".") {
  y <- as_chr(x, default = default)
  y[y == ""] <- default
  y
}

vcf_filter <- function(x) {
  y <- as_chr(x, default = "PASS")
  y[y == ""] <- "PASS"
  y
}
...
validate_input <- function(df) {
  ...
  if (length(missing) > 0) {
    fail("Input flat file is missing required columns: ", paste(missing, collapse = ", "))
  }
...
resolve_storage_option <- function(storage_option_name) {
  if (is.null(storage_option_name) || length(storage_option_name) == 0L ||
      is.na(storage_option_name) || storage_option_name == "") {
  ...
  obj <- tryCatch(
    ...
    error = function(e) {
      fail("Could not construct SeqArray storage option '", storage_option_name, "': ", e$message)
    }
  )
...
add_info <- function(gds, name, value, replace = TRUE) {
  SeqArray::seqAddValue(gds, paste0("annotation/info/", name), value,
                        replace = replace, verbose = FALSE)
}

add_func <- function(gds, name, value, replace = TRUE) {
  SeqArray::seqAddValue(gds, paste0("annotation/info/FunctionalAnnotation/", name), value,
                        replace = replace, verbose = FALSE)
}
...
ensure_functional_annotation_folder <- function(gds) {
  ...
  if (is.null(func_node)) {
    gdsfmt::addfolder.gdsn(info_node, "FunctionalAnnotation")
  }
  invisible(TRUE)
}
...
write_temp_vcf <- function(df, sample_id, temp_vcf) {
  ...
  writeLines("##FORMAT=<ID=GT,Number=1,Type=String,Description=\"Genotype reconstructed from FAVOR dosage\">", con)
  writeLines("##FORMAT=<ID=DS,Number=1,Type=Float,Description=\"Dosage from FAVOR genotype parquet for selected sample\">", con)

  for (chrom in unique(as_chr(df$chromosome))) {
    writeLines(paste0("##contig=<ID=", chrom, ">"), con)
  }

  writeLines(paste(c("#CHROM", "POS", "ID", "REF", "ALT", "QUAL", "FILTER", "INFO", "FORMAT", sample_id), collapse = "\t"), con)

  lines <- paste(
    as_chr(df$chromosome),
    as_int(df$position),
    id,
    as_chr(df$ref),
    as_chr(df$alt),
...
append_annotations <- function(gds_path, df, input_path) {
  ...
  # Keep variant.id aligned with the flat table for this chromosome/sample file.
  SeqArray::seqAddValue(gds, "variant.id", as_int(df$variant_id), replace = TRUE, verbose = FALSE)
  ...
  numeric_extras <- c(
    "matched_gene_score", "spliceai_max_ds", "alphamissense_max_pathogenicity",
    "revel", "gnomad_genome_af", "gnomad_exome_af", "bravo_af", "tg_all", "cadd_phred"
  )
  ...
  add_info(gds, "gds_contract_version", rep("impact_favor_flat_v4_dest_fixed", nvar))

  invisible(TRUE)
}

create_gds <- function(input_path, output_path, sample_id = NULL, force = FALSE,
                       keep_temp_vcf = FALSE, temp_vcf = NULL,
                       optimize = TRUE, storage_option = "ZIP_RA", verbose = FALSE) {
  ...
  if (is.null(storage_option) || length(storage_option) == 0L || is.na(storage_option) || storage_option == "") {
    storage_option <- "ZIP_RA"
  }

  log_msg("Resolved options:")
  log_msg("  input:           ", input_path)
  log_msg("  output:          ", output_path)
  log_msg("  sample_id:       ", ifelse(is.null(sample_id), "<infer from parquet>", sample_id))
  log_msg("  keep_temp_vcf:   ", keep_temp_vcf)
  log_msg("  temp_vcf:        ", ifelse(is.null(temp_vcf) || temp_vcf == "", "<auto>", temp_vcf))
  ...
  if (is.null(sample_id) || sample_id == "") {
    ...
    if (length(sample_values) != 1L) {
      fail("--sample-id was not provided and input contains multiple sample_id values: ", paste(sample_values, collapse = ", "))
    }
  ...
  if (is.null(temp_vcf) || temp_vcf == "") {
    temp_vcf <- tempfile(pattern = paste0(sample_id, "_"), fileext = ".vcf")
  }
  ...
  if (!keep_temp_vcf) {
    unlink(temp_vcf, force = TRUE)
  } else {
    log_msg("Temporary VCF retained: ", temp_vcf)
  }
  ...
  if (!identical(as.character(observed_sample), as.character(sample_id))) {
    fail("Post-write sample.id check failed")
  }
  if (observed_variants != nvar) {
    fail("Post-write variant count check failed: expected ", nvar, ", observed ", observed_variants)
  }
  if (length(observed_dosage) != nvar || length(observed_gene) != nvar) {
    fail("Post-write annotation length check failed")
  }
...
if (is.null(opt$input) || is.null(opt$output)) {
  fail("Both --input and --output are required. Use --help for usage.")
}
...
create_gds(
  ...
  force = opt$force,
  keep_temp_vcf = opt$keep_temp_vcf,
  temp_vcf = opt$temp_vcf,
  optimize = !opt$no_optimize,
  storage_option = opt$storage_option,
  verbose = opt$verbose
)
